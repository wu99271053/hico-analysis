...
bowtie2-build ./sacCer3/sacCer3.fa sacCer3_index
if [ $? -ne 0 ]; then
...
bowtie2 -x sacCer \
        -1 ${SRA_ID}/input/${SRA_ID}_input_1.fastq.gz \
        -2 ${SRA_ID}/input/${SRA_ID}_input_2.fastq.gz \
        -S ${SRA_ID}/input/${SRA_ID}.sam \
        $MODE \
        -p $THREADS
if [ $? -ne 0 ]; then
...
echo "Bowtie2 alignment completed successfully for $SRA_ID!"
